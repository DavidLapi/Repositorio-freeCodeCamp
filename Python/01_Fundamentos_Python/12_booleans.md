...
if is_citizen:
    if age >= 18:
        print('Has sido elegido para votar') # Salia: Has sido elegido para votar
else:
    print('No has sido elegido para votar')
```

El ejemplo anterior primero verificará si *is_citizen* es *True*. Si es así, pasará a la sentencia *if* anidada y verificará si *age* es mayor o igual a 18. Dado que *age* es mayor o igual a 18, el mensaje que se imprimirá en la terminal será *'Has sido elegido para votar'*. Si *is_citizen* fuera *False*, el mensaje que se imprimiría en la terminal habría sido *'No has sido elegido para votar'*.

Si estás trabajando con sentencias condicionales más complejas, puedes usar los operadores de Python *and*, *or* y *not*.

Pero antes de profundizar en esos operadores, veamos qué son los valores *truthy* (verdadero) y *falsy* (falso).
...
if is_citizen and age >= 18:
    print('Has sido elegido para votar') # Has sido elegido para votar
else:
    print('No has sido elegido para votar')
```
...
if age < 18 or is_student:
    print('Has sido elegido para un descuento de estudiante') # Has sido elegido para un descuento de estudiante
else:
    print('No has sido elegido para un descuento de estudiante')
```
...
```py
print(not '') # True, porque la cadena vacía es falsy
print(not 'Hello') # False, porque la cadena no vacía es truthy
print(not 0) # True, porque 0 es falsy
print(not 1) # False, porque 1 es truthy
print(not False) # True, porque False es falsy
print(not True) # False, porque True es truthy
```
...
if not is_admin:
    print('Acceso denegado para no administradores.') # Acceso denegado para no administradores.
else:
    print('Bienvenido, Administrador!')
```

Dado que *is_admin* es *False*, entonces *not is_admin* está diciendo no *False*, lo cual es *True*. Así que el mensaje 'Acceso denegado para no administradores.' será impreso.
...
- [X] You can watch the movie.
- [ ] SyntaxError
- [ ] TypeError
- [ ] You can't watch the movie.

2. ¿Cuál de los siguientes NO se considera un valor falsy en Python?
...
3. ¿Qué sucede cuando Python evalúa la expresión *x or y*?

- [ ] Devuelve *x* solo si *x* y *y* evalúan a *True*.
- [X] Devuelve *x* si evalúa a *True*, y omite evaluar *y*.
